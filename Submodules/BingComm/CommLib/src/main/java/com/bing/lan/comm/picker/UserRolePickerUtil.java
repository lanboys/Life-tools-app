...
import com.bing.lan.comm.picker.bean.UserRoleListBean;
import com.bing.lan.comm.utils.loader.AssetsLoader;

import java.util.ArrayList;

public class UserRolePickerUtil {

    PickerItemSelectListener mListener;

    private ArrayList<UserRoleListBean.UserRoleBean> options1Items1 = new ArrayList<>();

    private OptionsPickerView pvOptions;

    public UserRolePickerUtil(PickerItemSelectListener listener) {
        mListener = listener;
    }
    ...
    private void initOptionPicker(Activity activity) {//条件选择器初始化
        ...
        pvOptions = new OptionsPickerView.Builder(activity, new OptionsPickerView.OnOptionsSelectListener() {
            ...
            public void onOptionsSelect(int options1, int options2, int options3, View v) {
                ...
                .setCancelColor(AppUtil.getColor(R.color.colorPrimary))
                .setSubmitColor(AppUtil.getColor(R.color.colorPrimary))
    ...
    private void initOptionData1(Activity activity) {
        String json = AssetsLoader.loadJsonString(activity, "userrole.json");
        UserRoleListBean categoryBean = UserRoleListBean.objectFromData(json);

        options1Items1 = categoryBean.getList();
    }

    public interface PickerItemSelectListener {

        void onBankPickSelect(
                ArrayList<UserRoleListBean.UserRoleBean> options1Items1,
                int options1,
                View v);
    }
}
